...
int ristretto_add_string(ristretto *ris, char *s) {
  ris -> n_const += 1;
  fwrite(STRING_CONST, U2_SIZE, 1, ris -> out);
...
int ristretto_add_integer(ristretto *ris, unsigned int i) {
  ris -> n_const += 1;
  fwrite(INTEGER_CONST, U1_SIZE, 1, ris -> out);
  unsigned char d[] = "\x00\x00\x00";
  fwrite(d, U2_SIZE + U1_SIZE, 1, ris -> out);
  fwrite(&i, U1_SIZE, 1, ris -> out);
  ris -> index += 1;
  return 0;
}

int ristretto_add_class(ristretto *ris) {
  fwrite(CLASS, U2_SIZE, 1, ris -> out);
  fwrite(&(ris -> index), U1_SIZE, 1, ris -> out);
  ris -> index += 1;
  ris -> n_class += 1;
  return 0;
